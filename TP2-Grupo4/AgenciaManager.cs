...
//Listo de aca para abajo
        ...
        {
            return EliminarAlojamiento(codigo);
        ...
        public bool VerifyAvailableAccommodation(int codigoAlojamiento, int cantidadDePersonas)
        {
            ...
            using (MySqlConnection connection = Database.GetConnection())
            {
                try
                {
                    ...
                    while (reader.Read())
                    {
                        if(reader.GetInt32(0) >= cantidadDePersonas ) connection.Close(); return true;
                        connection.Close();
                        return false;
        ...
        {
            int indexUser = FindIndex(user => user.GetDni() == dni);
